...
C = {
    "ink": "#1F2D35", "navy": "#315B73", "navy2": "#24485D", "teal": "#2B8C82",
    "teal2": "#176B64", "cyan": "#75C0C8", "orange": "#D58A43", "orange2": "#A85F28",
    "sage": "#AFCBBD", "pale_teal": "#E9F4F0", "pale_blue": "#EBF2F6",
    "pale_orange": "#FBF1E7", "pale_gray": "#F4F6F7", "gray": "#6C7A82",
    "mid": "#B7C1C6", "grid": "#E5EAEC", "white": "#FFFFFF",
}

plt.rcParams.update({
    "font.family": "DejaVu Sans", "font.size": 10.0, "axes.titlesize": 11.5,
    "axes.labelsize": 10.0, "xtick.labelsize": 9.0, "ytick.labelsize": 9.0,
    "legend.fontsize": 8.8, "figure.facecolor": "white", "axes.facecolor": "white",
...
def figure1() -> None:
    ...
    y = np.arange(len(DATASETS)); vals = [DATASET_N[d] for d in DATASETS]
    ...
    for yi, v in zip(y, vals): ax.text(v*1.07, yi, f"{v:,}", va="center", fontsize=8.3)
    ax.legend(handles=[Rectangle((0,0),1,1,color=C["navy"],label="Classification"),
                       Rectangle((0,0),1,1,color=C["teal"],label="Regression")], frameon=False, loc="lower right")
    ...
    arrow(ax,(0.29,0.775),(0.39,0.775)); box(ax,(0.40,0.67),0.25,0.21,"Size-matched\nbaseline",C["pale_gray"],C["mid"],9.0,"bold")
    box(ax,(0.72,0.67),0.25,0.21,"Target-balanced\ncounterpart",C["pale_teal"],C["teal"],9.0,"bold"); arrow(ax,(0.66,0.775),(0.71,0.775),C["teal2"])
    ...
    arrow(ax,(0.37,0.62),(0.55,0.62),C["orange"]); ax.text(0.46,0.68,"deterministic\nselection",transform=ax.transAxes,ha="center",fontsize=8.0,color=C["gray"])
    ax.scatter([0.70],[0.62],s=[650],c=[C["teal"]],transform=ax.transAxes,clip_on=False); ax.text(0.70,0.84,"dominant fragment",transform=ax.transAxes,ha="center",fontweight="bold",fontsize=8.7)
    ...
    arrow(ax,(0.50,0.66),(0.50,0.56)); box(ax,(0.07,0.43),0.86,0.14,"Does predictive evidence change?",C["pale_teal"],C["teal"],9.0,"bold")
    arrow(ax,(0.50,0.41),(0.50,0.31)); box(ax,(0.07,0.16),0.39,0.13,"Scaffold-semantics\nperturbation",C["pale_orange"],C["orange"],8.5,"bold")
    box(ax,(0.54,0.16),0.39,0.13,"Molecular-representation\nperturbation",C["pale_orange"],C["orange"],8.5,"bold")
    save(fig,"figure1_audit_framework_v3")


def forest_panel(ax: plt.Axes, df: pd.DataFrame, task: str) -> pd.DataFrame:
    if task == "classification":
        order=[(d,m) for d in CLASSIFICATION for m in MODELS_CLS]; xlabel="AUC effect: balanced − size-matched"; bounds=[0,3,6,9,12]
    else:
        order=[(d,m) for d in REGRESSION for m in MODELS_REG]; xlabel="RMSE improvement: size-matched − balanced"; bounds=[0,3,6]
    rows=[]
    for d,m in order:
    ...
    for i in range(len(bounds)-1):
        if i%2==0: ax.axhspan(bounds[i]-0.5,bounds[i+1]-0.5,color=C["pale_gray"],zorder=0)
    ...
    for yi,mn,l,h,col in zip(y,means,lo,hi,colors):
        ax.plot([l,h],[yi,yi],color=col,lw=1.8,zorder=2); ax.plot([l,l],[yi-0.11,yi+0.11],color=col,lw=1.1); ax.plot([h,h],[yi-0.11,yi+0.11],color=col,lw=1.1)
        ax.scatter(mn,yi,s=38,color=col,edgecolor="white",linewidth=0.6,zorder=3)
    ax.axvline(0,color=C["gray"],ls="--",lw=1.1,zorder=1); ax.set_yticks(y,[f"{d} · {m}" for d,m in zip(p.dataset,p.model)]); ax.invert_yaxis(); ax.set_xlabel(xlabel); clean_axis(ax,"x")
    return p


def figure2() -> None:
    df=read_primary(); fig=plt.figure(figsize=(14.8,8.4)); gs=fig.add_gridspec(1,2,width_ratios=[1.35,1.0],wspace=0.34)
    fig.suptitle("Exact-size paired target-balance effects across 20 unique partition pairs",fontsize=15.2,fontweight="bold",y=0.985)
    ax1=fig.add_subplot(gs[0,0]); label_panel(ax1,"A","Classification · 12 dataset–model cells"); forest_panel(ax1,df[df.task_type=="classification"],"classification")
    ax2=fig.add_subplot(gs[0,1]); label_panel(ax2,"B","Regression · primary single-group semantics"); forest_panel(ax2,df[df.task_type=="regression"],"regression")
    fig.text(0.25,0.055,"0/12 corrected balanced advantages  ·  11/12 mean effects were negative",ha="center",fontsize=9.0,fontweight="bold",color=C["navy"])
    fig.text(0.74,0.055,"6/6 cells met the pre-specified corrected decision rule",ha="center",fontsize=9.0,fontweight="bold",color=C["teal2"])
    fig.subplots_adjust(bottom=0.13,top=0.90); save(fig,"figure2_primary_effects_v3")


def figure3() -> None:
    primary=read_primary(); single=read_singleton(); primary=primary[primary.dataset.isin(REGRESSION)].copy()
    fig=plt.figure(figsize=(14.8,8.6)); gs=fig.add_gridspec(2,2,height_ratios=[0.34,1.0],hspace=0.34,wspace=0.28)
    fig.suptitle("Regression effect depends on the structural semantics assigned to acyclic molecules",fontsize=15.0,fontweight="bold",y=0.985)
    ...
    box(ax,(0.06,0.47),0.34,0.26,"single-group\nall acyclic molecules → one identity",C["pale_blue"],C["navy"],9.2,"bold"); arrow(ax,(0.41,0.60),(0.59,0.60),C["orange"],1.7)
    ...
    for j,dataset in enumerate(REGRESSION):
        ax=fig.add_subplot(gs[1,j]); label_panel(ax,chr(ord('B')+j),dataset,x=-0.10,y=1.06); base=np.arange(3)
        for model_i,model in enumerate(MODELS_REG):
            pr=primary[(primary.dataset==dataset)&(primary.model==model)].iloc[0]; sr=single[(single.dataset==dataset)&(single.model==model)].iloc[0]
            for yoff,row,col,marker in [(-0.11,pr,C["navy"],'o'),(0.11,sr,C["orange"],'s')]:
                mn=float(row.mean_effect); lo=float(row.bootstrap_ci_low); hi=float(row.bootstrap_ci_high); yy=model_i+yoff
                ax.plot([lo,hi],[yy,yy],color=col,lw=1.8); ax.scatter(mn,yy,s=42,color=col,marker=marker,zorder=3)
        ax.axvline(0,color=C["gray"],ls='--',lw=1.0); ax.set_yticks(base,MODELS_REG); ax.invert_yaxis(); clean_axis(ax,"x"); ax.set_xlabel("RMSE improvement: size-matched − balanced")
    fig.legend(handles=legend,frameon=False,loc="lower center",ncol=2,bbox_to_anchor=(0.5,0.018))
    fig.text(0.28,0.065,"ESOL: primary gains strongly attenuate",ha="center",fontsize=9.0,fontweight="bold",color=C["orange2"])
    fig.text(0.74,0.065,"FreeSolv: all three singleton point estimates reverse sign",ha="center",fontsize=9.0,fontweight="bold",color=C["orange2"])
    fig.subplots_adjust(bottom=0.14,top=0.90); save(fig,"figure3_acyclic_sensitivity_v3")


def figure4() -> None:
    require(PARENT_COMPARISON); comp=pd.read_csv(PARENT_COMPARISON,keep_default_na=False)
    fig=plt.figure(figsize=(14.8,9.2)); gs=fig.add_gridspec(2,3,height_ratios=[0.48,1.0],wspace=0.30,hspace=0.38)
    fig.suptitle("Disconnected-component representation changes benchmark composition without changing corrected classification inference",fontsize=14.6,fontweight="bold",y=0.985)
    ax=fig.add_subplot(gs[0,0]); ax.set_axis_off(); label_panel(ax,"A","Representation perturbation")
    ax.scatter([0.18,0.29,0.38],[0.59,0.65,0.52],s=[330,190,100],c=[C["teal"],C["cyan"],C["orange"]],transform=ax.transAxes); arrow(ax,(0.49,0.60),(0.66,0.60),C["orange"])
    ax.scatter([0.79],[0.60],s=[620],c=[C["teal"]],transform=ax.transAxes); ax.text(0.27,0.82,"source-faithful",transform=ax.transAxes,ha="center",fontweight="bold",fontsize=8.6); ax.text(0.79,0.82,"dominant fragment",transform=ax.transAxes,ha="center",fontweight="bold",fontsize=8.6)
    box(ax,(0.15,0.15),0.70,0.16,"not a lossless formatting step",C["pale_orange"],C["orange"],8.7,"bold")
    affected=["BBBP","ClinTox","HIV"]
    ax=fig.add_subplot(gs[0,1]); label_panel(ax,"B","Multi-component records"); vals=[MULTICOMPONENT[d] for d in affected]; bars=ax.bar(affected,vals,color=[C["navy"],C["cyan"],C["teal"]],width=0.58); ax.set_yscale("log"); ax.set_ylabel("Count (log scale)"); clean_axis(ax,"y")
    for b,v in zip(bars,vals): ax.text(b.get_x()+b.get_width()/2,v*1.16,f"{v:,}",ha="center",fontsize=8.5)
    ax=fig.add_subplot(gs[0,2]); label_panel(ax,"C","Structural consequences"); x=np.arange(3); w=0.32
    b1=ax.bar(x-w/2,[SCAFFOLD_CHANGED[d] for d in affected],width=w,label="Scaffold changed",color=C["navy"]); b2=ax.bar(x+w/2,[SIM_LT_090[d] for d in affected],width=w,label="Similarity < 0.90",color=C["orange"])
    ...
    ax=fig.add_subplot(gs[1,:]); label_panel(ax,"D","Effect direction is representation-sensitive"); order=[(d,m) for d in affected for m in MODELS_CLS]; rows=[]
    for d,m in order:
        r=comp[(comp.dataset==d)&(comp.model==m)]
        if len(r)!=1: raise AssertionError(f"Missing parent comparison {d}/{m}")
        rows.append(r.iloc[0])
    p=pd.DataFrame(rows).reset_index(drop=True); y=np.arange(len(p)); main=p.main_mean_effect.astype(float).to_numpy(); parent=p.parent_mean_effect.astype(float).to_numpy(); reverse=np.sign(main)!=np.sign(parent)
    for yi,a,b,rev in zip(y,main,parent,reverse):
        ax.plot([a,b],[yi,yi],color=C["orange"] if rev else C["mid"],lw=1.8,zorder=1); ax.scatter(a,yi,s=42,color=C["navy"],zorder=2); ax.scatter(b,yi,s=42,color=C["orange"],marker='s',zorder=2)
    ax.axvline(0,color=C["gray"],ls='--',lw=1.0); ax.set_yticks(y,[f"{d} · {m}" for d,m in order]); ax.invert_yaxis(); clean_axis(ax,"x"); ax.set_xlabel("AUC effect: balanced − size-matched")
    ...
    fig.text(0.50,0.035,"7/9 mean-effect signs reverse   ·   9/9 corrected sensitivity inferences remain inconclusive",ha="center",fontsize=9.2,fontweight="bold",color=C["orange2"])
    fig.subplots_adjust(bottom=0.08,top=0.90); save(fig,"figure4_dominant_fragment_sensitivity_v3")


def figure5() -> None:
    fig=plt.figure(figsize=(14.8,9.2)); gs=fig.add_gridspec(2,2,wspace=0.28,hspace=0.38)
    fig.suptitle("Candidate-search budget is a benchmark-construction hyperparameter and was frozen before model fitting",fontsize=14.6,fontweight="bold",y=0.985)
    for j,d in enumerate(REGRESSION):
        ax=fig.add_subplot(gs[0,j]); label_panel(ax,chr(ord('A')+j),f"{d} · single-group acyclic semantics")
        budgets=np.array(list(BUDGET_SINGLE_GROUP[d])); vals=np.array(list(BUDGET_SINGLE_GROUP[d].values())); col=C["teal"] if d=="ESOL" else C["navy"]
        ax.plot(budgets,vals,color=col,marker='o',lw=2.0,ms=5); ax.fill_between(budgets,vals,0,color=C["pale_teal"] if d=="ESOL" else C["pale_blue"],alpha=0.75); ax.axvline(20000,color=C["orange"],ls='--',lw=1.2)
        ax.annotate("frozen cap\n20,000",xy=(20000,vals[-1]),xytext=(12500,vals.max()*0.58),arrowprops=dict(arrowstyle='->',color=C["orange"]),fontsize=8.3,color=C["orange2"])
    ...
    for d,col,marker in [("ESOL",C["teal"],'o'),("FreeSolv",C["orange"],'s')]:
        budgets=np.array(list(BUDGET_SINGLETON[d])); vals=np.array(list(BUDGET_SINGLETON[d].values())); ax.plot(budgets,vals/vals[0],color=col,marker=marker,lw=2.0,ms=5,label=d)
    ...
    y=np.arange(len(DATASETS)); vals=np.array([TEST_N[d] for d in DATASETS],float); ax.scatter(vals,y,s=85,color=C["navy"],label="size-matched",zorder=3); ax.scatter(vals,y,s=34,color=C["teal"],marker='s',label="target-balanced",zorder=4)
    ...
    fig.text(0.50,0.035,"Pre-outcome protocol audit: larger search spaces offer more opportunities for an extreme low-gap split; paired n_test is held exactly fixed.",ha="center",fontsize=9.0,fontweight="bold",color=C["gray"])
    fig.subplots_adjust(bottom=0.09,top=0.90); save(fig,"figure5_candidate_budget_audit_v3")
...
def figure6() -> None:
    ...
    fig.suptitle("The scientific claim changes at different levels when benchmark-construction rules are perturbed",fontsize=14.8,fontweight="bold",y=0.985)
    ...
    for i,(d,m) in enumerate(order):
        r=cls[(cls.dataset==d)&(cls.model==m)].iloc[0]; neg=float(r.mean_effect)<0; row=i//3; col=i%3; x=0.12+col*0.27; yy=0.62-row*0.12
        box(ax,(x,yy),0.19,0.075,m,C["pale_orange"] if neg else C["pale_teal"],C["orange"] if neg else C["teal"],7.8,"bold",radius=0.015)
        if col==0: ax.text(0.03,yy+0.037,d,transform=ax.transAxes,ha='right',va='center',fontsize=8.0,fontweight='bold')
    ...
    ax=fig.add_subplot(gs[0,1]); label_panel(ax,"B","Regression depends on scaffold semantics"); order=[(d,m) for d in REGRESSION for m in MODELS_REG]; y=np.arange(len(order))
    for yi,(d,m) in enumerate(order):
        pr=primary[(primary.dataset==d)&(primary.model==m)].iloc[0]; sr=single[(single.dataset==d)&(single.model==m)].iloc[0]; a=float(pr.mean_effect); b=float(sr.mean_effect); col=C["orange"] if np.sign(a)!=np.sign(b) else C["mid"]
        ax.plot([a,b],[yi,yi],color=col,lw=2.0); ax.scatter(a,yi,s=45,color=C["navy"]); ax.scatter(b,yi,s=45,color=C["orange"],marker='s')
    ax.axvline(0,color=C["gray"],ls='--',lw=1.0); ax.set_yticks(y,[f"{d} · {m}" for d,m in order]); ax.invert_yaxis(); clean_axis(ax,"x"); ax.set_xlabel("Effect (positive favors target balancing)")
    ...
    ax=fig.add_subplot(gs[0,2]); label_panel(ax,"C","Dominant-fragment sensitivity"); order=[(d,m) for d in ["BBBP","ClinTox","HIV"] for m in MODELS_CLS]; y=np.arange(len(order))
    for yi,(d,m) in enumerate(order):
        r=comp[(comp.dataset==d)&(comp.model==m)].iloc[0]; a=float(r.main_mean_effect); b=float(r.parent_mean_effect); rev=np.sign(a)!=np.sign(b)
        ax.annotate('',xy=(b,yi),xytext=(a,yi),arrowprops=dict(arrowstyle='-|>',lw=1.7,color=C["orange"] if rev else C["mid"],mutation_scale=11)); ax.scatter(a,yi,s=28,color=C["navy"],zorder=3)
    ax.axvline(0,color=C["gray"],ls='--',lw=1.0); ax.set_yticks(y,[f"{d} · {m}" for d,m in order]); ax.invert_yaxis(); clean_axis(ax,"x"); ax.set_xlabel("AUC effect")
    fig.text(0.50,0.055,"Regression: 6/6 supported under primary semantics; all three FreeSolv effects reverse under singleton.",ha="center",fontsize=8.8,fontweight="bold",color=C["teal2"])
    fig.text(0.82,0.025,"Dominant fragment: 7/9 directions reverse; 9/9 corrected inferences remain inconclusive.",ha="center",fontsize=8.6,fontweight="bold",color=C["orange2"])
    fig.subplots_adjust(bottom=0.13,top=0.90); save(fig,"figure6_claim_stability_map_v3")


def figure_s1() -> None:
    fig,ax=plt.subplots(figsize=(12.8,5.8)); x=np.arange(len(DATASETS)); raw=np.array([RAW_N[d] for d in DATASETS]); final=np.array([DATASET_N[d] for d in DATASETS]); removed=raw-final
    ax.bar(x,raw,color=C["pale_blue"],edgecolor=C["mid"],width=0.62,label="Raw rows"); ax.bar(x,final,color=[C["navy"] if d in CLASSIFICATION else C["teal"] for d in DATASETS],width=0.46,label="Final clean rows")
    ...
    ax.set_title("Audited raw-to-clean molecular-data construction",fontweight='bold',fontsize=14); ax.legend(frameon=False,ncol=2,loc='upper right'); save(fig,"figureS1_dataset_construction_v3")
...
def figure_s2() -> None:
    ...
    fig.suptitle("Candidate-budget behavior differs under alternative acyclic-scaffold semantics",fontsize=14,fontweight='bold'); fig.tight_layout(rect=[0,0,1,0.94]); save(fig,"figureS2_budget_semantics_v3")


def figure_s3() -> None:
    datasets=["BBBP","ClinTox","HIV"]; metrics=["Multi-component","Scaffold changed","Similarity < 0.90","Conflict groups"]
    mat=np.array([[MULTICOMPONENT[d],SCAFFOLD_CHANGED[d],SIM_LT_090[d],CONFLICT_GROUPS[d]] for d in datasets],float); score=np.log10(mat+1)
    fig,ax=plt.subplots(figsize=(10.8,5.8)); im=ax.imshow(score,cmap="YlGnBu",aspect='auto'); ax.set_xticks(np.arange(len(metrics)),metrics); ax.set_yticks(np.arange(len(datasets)),datasets); ax.set_title("Disconnected-component structural-audit summary",fontweight='bold',fontsize=14)
    for i in range(mat.shape[0]):
        for j in range(mat.shape[1]): ax.text(j,i,f"{int(mat[i,j]):,}",ha='center',va='center',fontsize=10,fontweight='bold',color=C["ink"] if score[i,j]<2.6 else C["white"])
