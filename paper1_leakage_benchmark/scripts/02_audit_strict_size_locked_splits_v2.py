...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit strictly size-locked scaffold pairs before retraining."
    )
...
def append_partition(
    *,
    ...
    row["nominal_target_test_n"] = meta.get("nominal_target_test_n")
    row["search_target_test_n"] = meta.get("target_test_n")
    row["reference_test_n"] = meta.get("reference_test_n")
    row["selection_priority"] = meta.get("selection_priority")
    row["strict_size_lock"] = meta.get("strict_size_lock")
    row["incumbent_source"] = meta.get("incumbent_source")
...
def main() -> None:
    ...
    for dataset in datasets:
        ...
        for seed in seeds:
            ...
            reference_n = int(size_meta["actual_test_n"])
            reference_scaffolds = sorted(
                size_df.loc[
                    size_df["split_size_matched_scaffold_v2"].eq("test"),
                    "scaffold",
            ...
            balanced_n = int(balanced_meta["actual_test_n"])
            pair_rows.append(
                {
                    "dataset": dataset,
                    "task_type": spec.task_type,
                    "partition_seed": seed,
                    "size_matched_n_test": reference_n,
                    "target_balanced_n_test": balanced_n,
                    "n_test_difference": balanced_n - reference_n,
                    "absolute_n_test_difference": abs(balanced_n - reference_n),
                    "exact_size_match": balanced_n == reference_n,
                    "size_matched_hash": size_meta["partition_hash"],
                    "target_balanced_hash": balanced_meta["partition_hash"],
                    "same_partition": (
                        size_meta["partition_hash"]
                        == balanced_meta["partition_hash"]
                    ),
                    "balanced_incumbent_source": balanced_meta.get(
                        "incumbent_source"
                    ),
                }
            )
    ...
    pair_summary = (
        ...
        .agg(
            ...
            max_absolute_n_test_difference=(
                "absolute_n_test_difference",
                "max",
            ),
            n_same_partitions=("same_partition", "sum"),
            n_reference_incumbents=(
                "balanced_incumbent_source",
                lambda values: int((values == "size_matched_reference").sum()),
            ),
        )
    )

    if not bool(pairs["exact_size_match"].all()):
        mismatches = pairs.loc[~pairs["exact_size_match"]]
        raise AssertionError(
            "Strict paired-size audit failed:\n"
            + mismatches.to_string(index=False)
        )

    suffix = f"strict_locked_{args.acyclic_mode}"
    paths = {
        ...
        "pairs": AUDIT_DIR / f"split_size_pairs_v2_{suffix}.csv",
        "pair_summary": AUDIT_DIR / f"split_size_pair_summary_v2_{suffix}.csv",
