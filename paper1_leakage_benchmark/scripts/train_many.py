...
def make_models(task_type: str, seed: int):
    if task_type == "classification":
        return {
            ...
            "LinearSVC": Pipeline([
                ("scaler", StandardScaler(with_mean=False)),
                ("model", LinearSVC(class_weight="balanced", random_state=seed, max_iter=5000)),
            ]),
        }
    return {
        ...
        "LinearSVR": Pipeline([
            ("scaler", StandardScaler(with_mean=False)),
            ("model", LinearSVR(random_state=seed, max_iter=5000)),
        ]),
    }
...
metric_cols = [c for c in ["accuracy", "f1", "roc_auc", "average_precision", "rmse", "mae", "r2"] if c in raw.columns]
summary = raw.groupby(["dataset", "task_type", "split", "model"], as_index=False)[metric_cols].agg(["mean", "std"])
...
gap_rows = []
for keys, group in raw.groupby(["dataset", "task_type", "seed", "model"]):
    dataset, task_type, seed, model = keys
    r = group[group["split"] == "random"]
    s = group[group["split"] == "scaffold"]
    if len(r) == 0 or len(s) == 0:
        continue
    if task_type == "classification":
        metric = "roc_auc"
        random_value = float(r.iloc[0][metric])
        scaffold_value = float(s.iloc[0][metric])
...
gap_summary = gap_raw.groupby(["dataset", "task_type", "model", "primary_metric"], as_index=False)["generalization_gap"].agg(["mean", "std"])
gap_summary.columns = ["_".join([x for x in col if x]) for col in gap_summary.columns.to_flat_index()]
