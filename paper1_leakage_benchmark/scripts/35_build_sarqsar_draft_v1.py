...
def formatted(row: pd.Series, digits: int = 4) -> str:
    mean = float(row["mean"])
    low = float(row["q025"])
    high = float(row["q975"])
    if not all(np.isfinite(value) for value in [mean, low, high]):
        return "NA"
    return f"{mean:.{digits}f} [{low:.{digits}f}, {high:.{digits}f}]"


def collect_values(summary: pd.DataFrame) -> dict[str, str]:
    values: dict[str, str] = {}
    for dataset in ["ESOL", "FreeSolv"]:
        for mode in ["single_group", "singleton"]:
            row = selected_row(summary, dataset, mode, "effect_rmse")
            values[f"{dataset}_{mode}"] = formatted(row)
    for metric, label in [
        ("effect_brier", "brier"),
        ("effect_log_loss", "logloss"),
        ("effect_roc_auc", "auc"),
    ]:
        numbers = [
            float(selected_row(summary, dataset, "single_group", metric)["mean"])
            for dataset in ["BACE", "BBBP", "ClinTox", "HIV"]
        ]
        finite = np.asarray(numbers, dtype=float)
        finite = finite[np.isfinite(finite)]
        values[f"{label}_range"] = "NA" if finite.size == 0 else f"{finite.min():.4f} to {finite.max():.4f}"
    return values


def build_tables(summary: pd.DataFrame) -> None:
    ...
    regression = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Maximum-budget endpoint-permutation null effects. Positive values favor response-aware selection.}",
        r"\label{tab:null-regression}",
        r"\small",
        r"\begin{tabular}{llrrr}",
        r"\hline",
        "Dataset & Acyclic rule & Draws & RMSE effect & MSE effect " + ROW_END,
        r"\hline",
    ]
    for dataset in ["ESOL", "FreeSolv"]:
        for mode in ["single_group", "singleton"]:
            rmse = selected_row(summary, dataset, mode, "effect_rmse")
            mse = selected_row(summary, dataset, mode, "effect_mse")
            regression.append(
                f"{dataset} & {mode.replace('_', ' ')} & {int(rmse['budget']):,} & "
                f"{float(rmse['mean']):.4f} & {float(mse['mean']):.4f} {ROW_END}"
            )
    regression.extend([r"\hline", r"\end{tabular}", r"\end{table}"])
    ...
    classification = [
        ...
        r"\hline",
        "Dataset & Draws & Brier & Log loss & ROC--AUC " + ROW_END,
        r"\hline",
    ]
    for dataset in ["BACE", "BBBP", "ClinTox", "HIV"]:
        ...
        classification.append(
            f"{dataset} & {int(brier['budget']):,} & {float(brier['mean']):.4f} & "
            f"{float(logloss['mean']):.4f} & {float(auc['mean']):.4f} {ROW_END}"
        )
    classification.extend([r"\hline", r"\end{tabular}", r"\end{table}"])
...
def build_sources(values: dict[str, str]) -> None:
    abstract = (
        ...
        f"At the maximum regression budget, null RMSE effects were {values['ESOL_single_group']} for ESOL and {values['FreeSolv_single_group']} for FreeSolv under the shared-acyclic convention, compared with {values['ESOL_singleton']} and {values['FreeSolv_singleton']} under singleton semantics. "
        ...
        "Together with the empirical audit, the simulations identify split-objective--metric coupling as a benchmark-design risk and motivate a minimum reporting checklist for response-aware QSAR validation."
    )
...
A QSAR benchmark is a measurement design rather than a neutral container for model scores. A particular risk arises when endpoint information enters split selection. For a constant predictor equal to the training mean,
\begin{equation}
\mathrm{MSE}=\mathrm{Var}(y_{\mathrm{test}})+(\bar y_{\mathrm{test}}-\bar y_{\mathrm{train}})^2.
\end{equation}
Reducing the second term can reduce RMSE even when molecular structure carries no signal. Binary Brier score has an analogous prevalence decomposition, whereas constant-score ROC--AUC has no direct prevalence-gap term.

\section{Materials and methods}
The analysis used BACE, BBBP, ClinTox, HIV, ESOL, and FreeSolv. Endpoint values were independently permuted 200 times within each dataset while preserving molecular rows, endpoint marginals, and scaffold geometry. The same 20 frozen partition seeds and nested target-blind candidate pools were used for size-matched and same-size response-aware selection. Partition-seed effects were averaged within each endpoint permutation, making the permutation the simulation replicate. Hard gates enforced exact-size pairing, non-worsening target gap, constant-score AUC invariance, complete cell counts, and the MSE decomposition.
...
\caption{Decomposition of the null MSE effect into squared mean-gap and test-variance contributions.}
\end{figure}
...
\section{Conclusion}
Split-objective--metric coupling is a benchmark-design risk. Molecular endpoint-permutation simulations show that response-aware exact-size scaffold selection can alter several metrics before molecular features contribute. Real-data conclusions are additionally conditional on scaffold semantics and molecular-record representation.

\section*{Data and code availability}
Public datasets, the frozen protocol, code, checkpointed simulation outputs, tables, figures, and manuscript-generation scripts are maintained in the project repository. An anonymized review archive will be prepared for double-blind peer review.
...
\end{document}
"""
    main = main_template.replace("<<TITLE>>", TITLE).replace("<<ABSTRACT>>", abstract)
    write(LATEX / "main.tex", main)

    title_template = r"""\documentclass[11pt]{article}
\usepackage[margin=2.6cm]{geometry}
\begin{document}
\begin{center}
...
\end{document}
"""
    write(LATEX / "title_page.tex", title_template.replace("<<TITLE>>", TITLE))

    supplementary_template = r"""\documentclass[11pt]{article}
...
\end{document}
"""
    write(LATEX / "supplementary.tex", supplementary_template.replace("<<TITLE>>", TITLE))

    notes = (
        "# SAR/QSAR scientific working draft\n\n"
        f"## Proposed title\n\n{TITLE}\n\n"
        ...
        f"- ESOL, shared acyclic group: {values['ESOL_single_group']}\n"
        f"- ESOL, singleton acyclic groups: {values['ESOL_singleton']}\n"
        f"- FreeSolv, shared acyclic group: {values['FreeSolv_single_group']}\n"
        f"- FreeSolv, singleton acyclic groups: {values['FreeSolv_singleton']}\n\n"
...
def main() -> None:
    args = parse_args()
    summary = pd.read_csv(
        require(TABLES / "null_metric_effect_summary.csv"),
        keep_default_na=False,
        na_values=["", "nan", "NaN"],
    )
    for name in [
        "figure_mc1_regression_null_coupling.pdf",
        "figure_mc2_classification_null_coupling.pdf",
        "figure_mc3_mse_decomposition.pdf",
    ]:
        require(FIGURES / name)
