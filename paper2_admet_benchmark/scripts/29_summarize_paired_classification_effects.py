...
"""Create a compact paired classification conformal effect summary.

The input contains one cross-seed paired estimate per frozen model/regime. This
script preserves the model-specific confidence intervals, reports descriptive
averages across the eight frozen model/regime combinations, and counts how many
model-specific intervals lie entirely above or below zero.
"""
...
def main() -> None:
    ...
    for keys, group in df.groupby(
        ...
        n_splits = pd.to_numeric(group["n_splits"], errors="coerce")
        if not (n_splits == expected_splits).all():
