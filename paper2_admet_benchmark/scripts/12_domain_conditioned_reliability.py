...
def compute_domain_conformal(
    ...
    for keys, group in merged.groupby(group_cols, sort=True):
        ...
        covered = group["covered"].astype(str).str.lower().map({"true": True, "false": False})
        if covered.isna().any():
        ...
        if task_type == "classification":
            row["mean_prediction_set_size"] = float(group["prediction_set_size"].mean())
            row["empty_set_rate"] = float(group["empty_set"].astype(str).str.lower().map({"true": True, "false": False}).mean())
            row["ambiguous_set_rate"] = float(group["ambiguous_set"].astype(str).str.lower().map({"true": True, "false": False}).mean())
        else:
...
def main() -> None:
    ...
    print("Domain-conditioned reliability analysis complete.")
