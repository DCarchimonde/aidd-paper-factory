...
\begin{figure}[p]
\centering
\includegraphics[width=0.99\textwidth]{figure1_audit_framework_v3.pdf}
\caption{Chemometric benchmark-audit framework. The source-faithful molecular universe, exact-size paired perturbation, pre-outcome freezing, acyclic-scaffold semantics, disconnected-component representation, and claim-stability tests are shown as distinct parts of one measurement workflow. The primary contrast uses the same dataset, partition seed, target-blind candidate pool, scaffold rule, and test-set cardinality; target-distribution mismatch is the designed perturbation.}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.99\textwidth]{figure2_primary_effects_v3.pdf}
\caption{Primary paired target-balance effects across 18 dataset--model cells. Points show mean paired effects over 20 unique partition pairs and horizontal bars show paired-bootstrap 95\% intervals. Positive effects favor target balancing for both task types. The classification panel emphasizes the absence of any corrected target-balanced advantage, whereas the regression panel shows the six supported effects under the primary single-group acyclic convention.}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.99\textwidth]{figure3_acyclic_sensitivity_v3.pdf}
\caption{Dependence of the regression effect on acyclic-scaffold semantics. Panel A isolates the semantic perturbation while holding endpoints, models, partition seeds, and exact-size pairing logic fixed. Panels B and C compare the primary single-group effects with the singleton sensitivity for ESOL and FreeSolv. The large primary gains attenuate strongly, and all three FreeSolv point estimates reverse sign.}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.99\textwidth]{figure4_dominant_fragment_sensitivity_v3.pdf}
\caption{Dominant-fragment representation sensitivity. Panels A--C summarize the representation perturbation and its structural consequences. Panel D connects source-faithful and dominant-fragment mean effects for the nine affected dataset--model cells. Seven of nine point-estimate signs reverse, whereas all nine corrected sensitivity inferences remain inconclusive.}
