...
Density-ratio weighting did not provide a comparable improvement. Across endpoints and splits, changes in minority coverage and class-conditional gap were small and rarely supported consistently across model families. For ClinTox, the descriptive positive-coverage increase ranged from 0.005 to 0.090, but no model-specific positive-coverage confidence interval was entirely above zero. Improved class-conditional coverage was therefore not equivalent to improved practical reliability: Mondrian produced a statistically stable class-level improvement, but at a substantial loss of informativeness, whereas shift weighting produced no stable class-level change.

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{figure_3_classification_conformal_tradeoff.pdf}
\caption{\textbf{Mondrian calibration improves class-conditional coverage at the cost of less informative prediction sets.} Panels A--F show positive- and negative-class empirical coverage for marginal, density-ratio shift-weighted, and Mondrian label-conditional conformal prediction at nominal 90\% coverage across BBBP and ClinTox split types. Panels G and H summarize mean prediction-set size and ambiguous two-label set rate for the same endpoint--split--method combinations. R, S, and C denote random, scaffold, and similarity-cluster splits. The dashed line indicates nominal coverage.}
